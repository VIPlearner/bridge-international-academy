...
class GetSyncStateUseCaseTest {
    ...
    @Test
    fun `invoke returns OUT_OF_DATE when repository returns OUT_OF_DATE`() = runTest {
    ...
    @Test
    fun `invoke returns UP_TO_DATE when repository returns UP_TO_DATE`() = runTest {
    ...
    @Test
    fun `invoke returns SYNCING when repository returns SYNCING`() = runTest {
    ...
    @Test
    fun `invoke returns flow that emits multiple values when repository flow changes`() = runTest {
        val flowValues = mutableListOf<SyncState>()
        every { dataStoreRepository.getPupilSyncState() } returns flowOf(
